...
- a026423: Group leave, local movie search, and UI fixes
  - Added ability for members to leave groups directly from the group page with confirmation modal
  - Added local text search with 300ms debounce for filtering movies within a group
  - Fixed infinite loading spinner that persisted when clearing the movie search query
  - Fixed movie description rendering to preserve line breaks instead of collapsing them
...
- 976ba36: Added year filter to movie search and improved page actions
  - Added year range filter to movie search with debounced input, normalized query keys, deduplicated requests, and improved loading states
  - Moved delete actions from edit pages to detail pages
  - Added dropdown menu support for multiple trailing actions

### Patch Changes

- Updated dependencies [976ba36]
  - @repo/ui@0.11.0
...
- 354709b: Adapt frontend to consolidated watchDate field
  - Updated validation, mapper, and types to align with the new nullable watchDate field
  - Replaced $derived.by with $derived in MovieCard
...
- 7656a50: Movie detail pages with create/edit functionality and role-based access control
