...
### Паттерн тегов: список vs элемент

**ВАЖНО:** Разделяйте теги для списка и отдельного элемента, чтобы избежать дублирования запросов.

| Сущность | Key | Tags | Что инвалидирует |
|----------|-----|------|------------------|
| Список | `['items']` | `['items']` | `invalidateByTag('items')` |
| Элемент | `['item', id]` | `['item']` | `invalidateByKey(['item', id])` |

**Mutation для обновления элемента:**
```typescript
createMutation({
	tags: ['items'],                                      // Инвалидирует список
	invalidateKeys: (_, { id }) => [['item', id]],        // Инвалидирует конкретный элемент
});
```

**Почему это важно:**

Если query элемента имеет тег `items` (как у списка), то при mutation:
1. `invalidateByTag('items')` — инвалидирует и список, и элемент
2. `invalidateByKey(['item', id])` — снова инвалидирует элемент

Результат: **два запроса** на один query.

**Правильно:**
```typescript
// ✅ Query элемента — тег 'item' (единственное число)
createQuery({
	key: ['item'],
	tags: ['item'],  // НЕ 'items'!
	// ...
...
// ✅ Mutation — инвалидатирует список по тегу, элемент по ключу
createMutation({
	tags: ['items'],
	invalidateKeys: (_, { id }) => [['item', id]],
	// ...
});
```
...
**Неправильно:**
```typescript
// ❌ Query элемента с тегом списка — приведёт к дублированию запросов
createQuery({
	key: ['item'],
	tags: ['items'],  // НЕПРАВИЛЬНО!
	// ...
});
```
